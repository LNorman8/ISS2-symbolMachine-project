%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% PARALLEL HYPERPARAMETER SWEEP FOR ADAPTIVE EXPERT MIXTURE (S26)
%%% Uses parfor to accelerate validation-based tuning.
...
% Parallel settings
useParallel = false;
requestedWorkers = [];   % [] = MATLAB default, or set e.g. 8
...
if useParallel
    ...
    if isempty(p)
        ...
        if ~isempty(requestedWorkers) && p.NumWorkers ~= requestedWorkers
            delete(p);
            parpool(requestedWorkers);
        end
    end
...
if useParallel
    % parfor-friendly outputs
    bpsVec   = zeros(numCombos,1);
    bitsVec  = zeros(numCombos,1);
    nVec     = zeros(numCombos,1);
    ...
    for i = 1:numCombos
        results(i).id = i;
        results(i).bitsPerSymbol = bpsVec(i);
        results(i).totalBits = bitsVec(i);
        results(i).numSymbols = nVec(i);
