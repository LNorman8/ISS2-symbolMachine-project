function [symbol,penaltyInBits] = symbolMachineS26(probs)
% function [symbol,penaltyInBits] = symbolMachineS26(probs)
%
% Runs one step of the Symbol Machine. After initializing the Symbol
% Machine with initializeSymbolMachineS26.m, symbolMachineS26.m should be
...
%       to appear in the sequence. These should be nonnegative and sum to
%       one.
%
% Outputs:
...
%
% Note: initializeSymbolMachineS26.m must be used before the first call to
%     symbolMachineS26.m.
%
...
if strcmp(SYMBOLDATA.initializerVersion,'S26')
    SYMBOLDATA.machineVersion = 'S26';
else
    error('Symbol Machine should first be initialized with initializeSymbolMachineS26.m.');
end
...
if SYMBOLDATA.nextIndex <= SYMBOLDATA.sequenceLength
    ...
    error('No more symbols available. Call initializeSymbolMachineS26.m if needed.');
end
